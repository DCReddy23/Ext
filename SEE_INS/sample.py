# --- Basic bit helpers ----
def rotr(x, n):
    return ((x >> n) | (x << (32 - n))) & 0xFFFFFFFF

def shr(x, n):
    return x >> n

# Logical functions
def ch(x, y, z):
    return (x & y) ^ (~x & z)

def maj(x, y, z):
    return (x & y) ^ (x & z) ^ (y & z)

# Mix functions (renamed simple)
def big0(x):   return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22)
def big1(x):   return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25)
def small0(x): return rotr(x, 7) ^ rotr(x, 18) ^ shr(x, 3)
def small1(x): return rotr(x, 17) ^ rotr(x, 19) ^ shr(x, 10)

# SHA-256 Constants
K = [
  0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
  0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
  0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
  0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
  0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
  0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
  0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
  0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
]

# Initial Hash
H0 = [
  0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
  0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19
]

def sha256(message):
    msg = bytearray(message)
    bit_len = len(msg) * 8

    # Pad message
    msg.append(0x80)
    while len(msg) % 64 != 56:
        msg.append(0)
    msg += bit_len.to_bytes(8, 'big')

    H = H0[:]

    # Process each 512-bit block
    for i in range(0, len(msg), 64):
        block = msg[i:i+64]

        # Prepare message schedule
        w = [0]*64
        for t in range(16):
            w[t] = int.from_bytes(block[t*4:t*4+4], 'big')
        for t in range(16, 64):
            w[t] = (w[t-16] + small0(w[t-15]) + w[t-7] + small1(w[t-2])) & 0xFFFFFFFF

        a,b,c,d,e,f,g,h = H

        # Compression loop
        for t in range(64):
            T1 = (h + big1(e) + ch(e,f,g) + K[t] + w[t]) & 0xFFFFFFFF
            T2 = (big0(a) + maj(a,b,c)) & 0xFFFFFFFF

            h, g, f, e, d, c, b, a = (
                g, f, e, (d + T1) & 0xFFFFFFFF, c, b, a, (T1 + T2) & 0xFFFFFFFF
            )

        # Update hash
        H = [(H[j] + v) & 0xFFFFFFFF for j, v in enumerate([a,b,c,d,e,f,g,h])]

    return ''.join(f"{x:08x}" for x in H)


# Tests
print("SHA256('')    =", sha256(b""))
print("SHA256('abc') =", sha256(b"aneesh"))
